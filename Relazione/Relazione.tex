...
\begin{document}
	...
	\section{Profilazione}
		...
		Inoltre nella tabella \ref{tab:nvprof} è riportata la durata in tempo e in percentuale e il numero di chiamate 
		effettuate per ogni kernel (profilazione avvenuta con un array di input con dimensione $134217728=2^{27}$).
		\begin{table}[h]			
			\centering
			\begin{tabular}{c|ccc}
				                  & bitonic\_sort & bitonic\_merge & bitonic\_merge\_2 \\
			...
			\end{tabular}
			\caption{Tabella di profilazione}
			\label{tab:nvprof}
		\end{table}
